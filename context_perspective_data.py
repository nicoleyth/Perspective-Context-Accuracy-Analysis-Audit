...
def main():
    # Load the dataset
    df = pd.read_csv("dataset/context_dataset/gc.csv") 

    # Store toxicity scores
    toxicity_scores = []
    toxicity_w_context = []

    os.makedirs('dataset', exist_ok=True)

    with open('dataset/context_perspective_score.csv', 'w', newline='') as csvfile:
    ...
    df = pd.read_csv("data/context_perspective_score.csv")
    print("\nScore Summary:")
...
@inproceedings{xenos-etal-2021-context,
    ...
    a dataset of 10k posts with two kinds of toxicity labels per post, obtained from annotators who considered (i) both
    the current post and the previous one as context, or (ii) only the current post. We introduce a new task, 
    context-sensitivity estimation, which aims to identify posts whose perceived toxicity changes if the context (previous post)
    is also considered. Using the new dataset, we show that systems can be developed for this task. Such systems could be used to
    enhance toxicity detection datasets with more context-dependent posts or to suggest when moderators should consider the parent
    posts, which may not always be necessary and may introduce additional costs.",
}
"""